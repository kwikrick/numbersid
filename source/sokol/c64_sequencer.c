...
void app_init(void) {
    ...
    fs_init();
        ...
        ui_init(&(ui_desc_t){
            ...
            .imgui_ini_key = "floooh.chips.c64_sequencer",
        });
...
static void ui_save_snapshot(size_t slot) {
    if (slot < UI_SNAPSHOT_MAX_SLOTS) {
        ...
        fs_save_snapshot("c64_sequencer", slot, (chips_range_t){ .ptr = &state.snapshots[slot], sizeof(c64_snapshot_t) });
    }
}
...
static void ui_load_snapshots_from_storage(void) {
    for (size_t snapshot_slot = 0; snapshot_slot < UI_SNAPSHOT_MAX_SLOTS; snapshot_slot++) {
        fs_load_snapshot_async("c64_sequencer", snapshot_slot, ui_fetch_snapshot_callback);
    }
}
